...
let createSlide = (req, res) => {
  ...
  // Sanity check
  //image = typeof(image) === 'string' && image.trim().length > 0 || image instanceof String ? image.trim() : null;
  title = typeof(title) === 'string' && title.trim().length > 0 || title instanceof String ? title.trim() : undefined;
  
  description = typeof(description) === 'string' && description.trim().length > 0 || description instanceof String ? description.trim() : undefined;
...
const editSlide =  (req,res) => {
  ...
  Slide.findById(id, (err, data) => {
    ...
    // 2.
    let validateArchiveChange = (req,imageRoute) => {
      return new Promise((resolve, reject) => {
        ...
        if(req.files){
          ...
          let extension = archive.name.split(".").pop();
    ...
    // 3. 
    let changeDBREgistry = (id, title, description, imageRoute) => {
      return new Promise((resolve, reject) => {
        ...
        Slide.findByIdAndUpdate(id, newData, { new: true, runValidators:true}, (err, data) => {
          if(err){
    ...
    // sincronizar
    validateArchiveChange(req, imageRoute).then(imageRoute => {
      changeDBREgistry(id, body, imageRoute).then(response => {
        ...
        response.res.status(400).send({
          err: response.err,
          message: 'Could not edit the slide'
