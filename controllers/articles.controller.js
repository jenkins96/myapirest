...
const createArticle = (req, res) => {
  ...
  if (!title || !intro || !url || !content) {
    ...
    archive.mv(`./archives/articles/${url}/${name}.${extension}`, (err) => {
      if (err)
        res.status(500).send({ message: "Could not save the image", err });
      ...
      article.save((err, data) => {
        if (err)
          res.status(500).send({ message: "Could not save the article" });

        return res
          .status(201)
          .send({ message: "Article saved successfully", data });
      });
    });
  }
};
  ...
  Articles.findById(id, (err, data) => {
    ...
    const validateArchiveChange = (req, cover) => {
      return new Promise((resolve, reject) => {
        if (req.files) {
          ...
          const directoryCreation = mkdirp.sync(`./archives/articles/${url}`);
          archive.mv(
            `./archives/articles/${url}/${name}.${extension}`,
            (err) => {
              if (err) {
                const response = {
                  res,
    ...
    const asyncCall = async (req, id, cover, title, intro, url, content) => {
      try {
        ...
        let responseChangeDb = await changeDBREgistry(
          id,
          cover,
          title,
          intro,
